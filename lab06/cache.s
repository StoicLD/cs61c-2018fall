...
main:	li	a0, 256		# array size in BYTES (power of 2 < array size)
	li	a1, 8		# step size  (power of 2 > 0) step size is in words
...
accessWords:
	la	s0, array		# ptr to array    ,limit 10
	add	s1, s0, a0		# hardcode array limit (ptr) a0 is the size of array in bytes
	slli	t1, a1, 2		# multiply stepsize by 4 because WORDS
wordLoop:
	beq	a3, zero,  wordZero		#option的选择，如果是0就改成0，如果是1就加一

	lw	t0, 0(s0)		# array[index/4]++   把数组内容加载到t0
	addi	t0, t0, 1	#加一操作，option为1的情况
	sw	t0, 0(s0)
	j	wordCheck
...
wordCheck:
	...
	blt	s0, s1, wordLoop	# inner loop done? judge whether out of index
	
	addi	a2, a2, -1
	bgtz	a2, accessWords	# outer loop done?  branch great than zero? no,then stop outer loop
	jr	ra
